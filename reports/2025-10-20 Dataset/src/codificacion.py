...
def decodificar_categoria(valor_codificado, dict_codificacion, columna):
    """
    Convierte un valor codificado numéricamente a su representación original de texto.

    Parámetros:
        valor_codificado : int
            El valor codificado (numérico).
        dict_codificacion : dict
            Diccionario de mapeos {columna: {valor_original: código}}.
        columna : str
            Nombre de la columna a decodificar.

    Retorno:
        El valor original (str) si existe en el diccionario, en caso contrario devuelve el valor tal cual.
    """
    if columna in dict_codificacion:
        inv_map = {v: k for k, v in dict_codificacion[columna].items()}
        return inv_map.get(valor_codificado, valor_codificado)
