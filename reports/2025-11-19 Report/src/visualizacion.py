...
def graficos_por_columna(
    ...
    log_y=False,
    use_log=True,
    save_path=None,     # <-- NUEVO: ruta opcional para guardar PNG
    mostrar=True        # <-- NUEVO: mostrar o no en pantalla
):
    """
    ...
    Parámetros nuevos:
    - save_path: ruta completa del archivo PNG a guardar (o None para no guardar).
    - mostrar: si True hace plt.show(); si False no lo muestra (útil para Streamlit).
    """
    ...
    for i, param in enumerate(columnas_validas):
        ...
        if tipo == "histograma":
            ...
            if df_sin_error is None:
                ax.pie(
                    err_plot,
                    labels=[str(c) for c in all_index],
                    autopct='%1.1f%%',
                    startangle=90,
                    colors=plt.cm.Reds(np.linspace(0.4, 0.9, len(all_index)))
                )
                ...
                ax.pie(
                    ...
                    autopct='%1.1f%%',
                    startangle=90,
                    colors=plt.cm.Blues(np.linspace(0.4, 0.9, len(all_index)))
                )
    ...
    return parametros_constantes, fig
